...
export function computeHours(
  ...
  const rawMin = Math.max(0, differenceInMinutes(new Date(clockOutMs), new Date(clockInMs)));
  const step = rounding === "nearest-15" ? 15 : rounding === "nearest-5" ? 5 : 1;
  const roundedMin = Math.round(rawMin / step) * step;
  return Math.round((roundedMin / 60) * 100) / 100;
}

export function derivePeriodId(atMs: number, period: "weekly" | "biweekly" | "monthly"): string {
